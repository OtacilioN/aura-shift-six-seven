# Derivar Ascensão da Aura Ascendida acumulada

> Substituída por [ADR-0014](0014-custo-quadratico-por-multiplicador-de-ascensao.md) em 27 de julho de 2026. Este arquivo preserva a decisão histórica de `balance-v0.2` e `balance-v0.3`.

O `balance-v0.2` substitui parcelas independentes por um acumulado permanente `L` de toda Aura da Jornada consolidada como Aura Ascendida. O multiplicador em centésimos passa a ser `A(L)=100+floor(√(L÷10¹¹))`, e cada Ascensão mostra `A(L+J)−A(L)`. Assim, a primeira Ascensão em `1Qa` ainda dobra a produção, mas dividir `4Qa` em quatro resets produz o mesmo `3×` que uma jornada única de `4Qa`. O farm deixa de premiar a frequência de resets com crescimento aproximadamente exponencial e passa a crescer de forma aproximadamente linear no calendário idealizado.

Saves anteriores não registram a Aura Ascendida de cada jornada. A migração estima `L` uma vez a partir da contagem `N` e do bônus antigo `B=max(0,A−100)`: para `N>0`, `L=max(N×1Qa, floor(B²×10¹¹/N))`; para `N=0`, `L=B²×10¹¹`. Essa aproximação não recupera jornadas históricas desiguais com perfeição, mas normaliza o multiplicador para a curva nova sem apagar saldos, Aura Total, Resto, coleção, conquistas ou a Taxa Offline Registrada.
