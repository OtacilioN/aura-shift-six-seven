...
O multiplicador começa em `1×` e é derivado da Aura Ascendida acumulada `L`. Em `balance-v0.4`, o requisito do candidato `A` em centésimos é `L_req(A)=ceil(1Qa×(A−100)×A×(2A−100)/6.000.000)`, e vale o maior `A` cujo requisito não supera `L`. Cada jornada exige ao menos `1Qa` e acrescenta toda a Aura da Jornada a `L`. A primeira Ascensão mínima leva o total a `2×`; `10×→11×` exige `100Qa`, dez vezes o tempo-base no modelo ideal com produção em `10×`. Particionar a mesma Aura continua sem gerar vantagem.
...
3. carregar todos os custos e contribuições versionados em `balance-v0.4`;
...
### Orçamentos de Gate do `balance-v0.4`
...
Suas Contribuições-base são `1`, `5`, `500`, `500K`, `500M` e `500B Aura/ciclo por nível`. Em `balance-v0.4`, os custos continuam subindo `1.000×` por Técnica depois de `TECH-02`, e `TECH-03` a `TECH-06` também crescem `1.000×` entre si. A razão `Contribuição-base ÷ custo-base` é `5/67` em `TECH-02` e `1/134` em cada Técnica de `TECH-03` a `TECH-06`. A matriz deve medir tanto o nerf uniforme de `10×` das Técnicas `03` a `06`, herdado de `balance-v0.3`, quanto a nova curva de Ascensão.
